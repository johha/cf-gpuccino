...
A Cloud Foundry buildpack that configures GPU-enabled application containers at staging time. It sets the necessary environment variables (`CUDA_VISIBLE_DEVICES`, `LD_LIBRARY_PATH`, `HIP_VISIBLE_DEVICES`) and installs GPU-specific Python dependencies.
...
### 2. Push with the GPU buildpack

```bash
cf push myapp \
  -b https://github.com/johha/cf-gpuccino/buildpack/gpu-buildpack \
  -b python_buildpack
```
...
| Script | Purpose |
|---|---|
| `bin/detect` | Exits 0 if `.gpu-enabled` or `requirements-gpu.txt` is present |
| `bin/compile` | Writes `.profile.d/gpu-env.sh`; installs `requirements-gpu.txt` if present |
| `bin/release` | Outputs default process type YAML |
